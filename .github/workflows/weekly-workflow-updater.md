---
name: Weekly Workflow Updater
description: Automatically updates GitHub Actions versions weekly and creates a PR if changes are detected
on:
  schedule:
...
> **Prerequisite**: This workflow requires the `gh aw` CLI extension. Ensure
> `.github/workflows/copilot-setup-steps.yml` includes the `gh aw` installation step.

## Your Mission

Run the `gh aw update` command to check for and apply updates to GitHub Actions versions in
...
Look specifically for changes to `.github/aw/actions-lock.json`. We only want to create a PR if
...
**CRITICAL**: Do NOT include `.lock.yml` files in the PR. These files are compiled workflow files
and should not be committed as part of action updates.
...
This PR updates GitHub Actions versions in `.github/aw/actions-lock.json` to their latest
compatible releases.
...
- Workflow `.lock.yml` files are excluded from this PR and will be regenerated during the next
  compilation

### Testing

The updated actions will be automatically used in workflow compilations. No manual testing
required.
...
- **No updates available**: If `actions-lock.json` was not modified, do NOT create a PR. Exit
  gracefully with a message like "All actions are already up to date."

- **Only .lock.yml files changed**: If only `.lock.yml` files changed but `actions-lock.json` was
  not modified, reset the lock files and exit without creating a PR.

- **Update command fails**: If the `gh aw update` command fails, report the error but do not
  create a PR. The error might be temporary (network issues, API rate limits).
...
- Updates are checked weekly (Sundays)
